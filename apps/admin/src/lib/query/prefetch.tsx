...
function isInfiniteQueryOptions(option: object): option is PrefetchInfinite {
  return "getNextPageParam" in option;
}
 ...
 * and infinite queue factories (`getNextPageParam` → `prefetchInfiniteQuery`).
 */
export async function prefetchAdmin(options: readonly object[], children: ReactNode) {
  const client = makeQueryClient();
  await Promise.all(
    options.map((option) =>
      isInfiniteQueryOptions(option)
        ? client.prefetchInfiniteQuery(option)

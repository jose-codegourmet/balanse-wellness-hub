      ...
      >
        <ScrollReveal>
          <div className="coaches-roster-heading">
            ...
            <p>
              Coaches you will see every week, not a rotating cast. Start with a familiar practice
              or let someone introduce you to a new one — every name below is teaching on the public
              calendar.
            </p>
          </div>
        </ScrollReveal>
        ...
        {visible.length ? (
          <ul className="coaches-roster-grid">
            {visible.map((coach, index) => (
              <li key={coach.id}>
                <ScrollReveal delay={Math.min(index, 5) * 0.06} className="h-full">
                  <article

...
9. Mock-phase UI uses `getMockAdapter()` (admin through `apps/admin/src/lib/query/`). No UI `/api/*` or Supabase calls. `MockDataAdapter` implements the mock ledger. Backend Prisma, RLS, and HTTP handlers are BE-058 and stay unwired from screens. See `docs/backend/session-bundles.md`.
...
- HTTP (implemented, not wired): `GET /api/public/packages`, `GET /api/me/packages`, `POST /api/packages/{id}/claim`, `POST /api/admin/bundles`, grant/revoke and paid-acquisition review

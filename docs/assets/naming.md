...
Blur-up derivative (local only, 480px long edge): `docs/assets/marketing/{page}/{slot}-{aspect}-thumb.{webp,jpg}`. Thumbs are not uploaded, so they get no `storage_key` and no manifest `storage_objects` row.
```

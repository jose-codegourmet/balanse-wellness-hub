...
For this track, generate images with Higgsfield **`nano_banana_pro` only**, at **`1k` resolution**.
...
2. GENERATE    Higgsfield run (nano_banana_pro @ 1k) that takes catalogued source as reference when required
...
| --- | --- | --- | --- |
| All production gens this track | `nano_banana_pro` | **1k** | Jose / TL lock |
...
| Client | Coach Rex (headshots + look lockup); Jose / Rex for marketing pages | `client-review` → `approved` or `rejected` |
...
| Spec ratio | Native on `nano_banana_pro`? | Procedure |
| --- | --- | --- |
...
| `soul_2` + `4:5` (if policy ever changes) | No | Generate `3:4`, then `reframe`/crop to `4:5`. Prefer staying on `nano_banana_pro`. |

### Generate-then-crop (3:1)

1. Generate About C at `21:9`, `nano_banana_pro`, `1k`.

...
UI and `MockDataAdapter` already implement the product rules on the mock harness. This phase also ships **schema, HTTP contracts, transactional SQL**. Screens stay mock-first: do **not** wire screens to `/api/*` or Supabase until a dedicated WIRE ticket.

## Product invariants

- Remaining sessions = `snapshotSessionCreditCount − count(HELD + CONSUMED)` (HTTP) or granted minus held/consumed redemptions on the mock ledger. Never trust an unaudited mutable counter.
...
- **Waitlist promotion block:** if the intended entitlement is exhausted, expired, revoked, ineligible, or foreign, the waiter stays waitlisted, `packagePromotionBlocked` / `promotionBlockReason` is set, and later FIFO rows may promote. The app never silently charges cash or picks another package.
...
See `packages/db/contracts/routes.ts` ticket `BE-058`.
...
`packages/mock/src/bundle-fixtures.ts` backs the shipped screens via `getMockAdapter()` (`getPublicBundles`, `getMyEntitlements`, admin bundle CRUD). Runtime flags include `failFinalCredit` and `emptyPackageCatalogue`.

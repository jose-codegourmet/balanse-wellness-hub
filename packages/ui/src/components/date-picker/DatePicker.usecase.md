...
- Range panel is two months only when **both** the viewport and the picker container are `≥ 768px` (`BALANSE_BREAKPOINTS.tablet`); otherwise one month so a 360px story / narrow filter does not overflow. Presets use `manilaYmd`, `startOfManilaWeekMonday`, `startOfManilaMonth`, `addManilaDays`, and `daysInManilaMonth` — no inline date arithmetic.
...
- Enter / Space on the **calendar icon** opens the panel (that button is the `PopoverTrigger`). Enter in the text field commits a typed value and does not open the panel — and it `preventDefault`s so it will not submit a parent form while the input is focused.
